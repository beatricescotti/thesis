...
In this chapter, the results obtained from the various training strategies and models are presented. The discussion will start with the results from the medical images, confronting the model's performances with different types of preprocessing described in Chapter 4. Then, the results from the jet images training will be presented, confronting the curriculum learning strategy with traditional training, going on with analyzing the impact of freezing different layers of the model during training and finishing with the transfer learning results on medical images.
The model used for all training is the eight's version of YOLO, explained in Chapter 2. 

\section{Medical Images}
The medical images used in this thesis are CT scans from the NLST dataset, which contains annotated lung nodules. The annotations were made by expert radiologists and include bounding boxes around the nodules, as well as additional information such as nodule size and location. We have approximately 9000 annotated images, which are sufficient to train a model.
As explained in Chapter 4, different types  of preprocessing were applied to the image to enhance the features of interest, and a model was trained on the full dataset to establish a performance baseline and to compare the different types of preprocessing. 
The model weights were initialized with the COCO pre-trained weights, and the hyperparameters used for training were as follows:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Linear conversion + masking} the lung region, mantaining the original pixel size, 512x512.
    ...
    \item \textbf{Linear conversion + resampling + padding} to 704x704 pixels.
    \item \textbf{Windowing + resampling + padding} to 704x704 pixels.
\end{enumerate}
The results of the different types of preprocessing are shown in Figure~\ref{fig:preprocessing_results}, which shows the mAP@50 evaluated on the validation set after each epoch during training.
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.8\textwidth]{immagini/cap7/mAP50B_preprocessing_padding.png}
    \caption{mAP@50 evaluated on the validation set after each training epoch for the different types of preprocessing.}
    \label{fig:preprocessing_results}
\end{figure}

Every dataset has their own test set, composed of approximately 900 images, and the model with the best mAP@50 on the validation set during training was saved and used for evaluation on the test set. The results are summarized in the following table \ref{tab:medical_results}:

\begin{table}[!ht]
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{|c|c|c|}
    \hline
    \textbf{Model}                                                                 & \textbf{Test mAP@50}         & \textbf{Validation mAP@50}   \\ \hline
    {\color[HTML]{3531FF} Linear conversion (0-255) {[}512, 512{]}}                & {\color[HTML]{3531FF} 0.631} & {\color[HTML]{3531FF} 0.771} \\
    {\color[HTML]{F8A102} Linear conversion + masking {[}512, 512{]}}              & {\color[HTML]{F8A102} 0.752} & {\color[HTML]{F8A102} 0.637} \\
    {\color[HTML]{009901} RGB windowing {[}512, 512{]}}                            & {\color[HTML]{009901} 0.770} & {\color[HTML]{009901} 0.729} \\
    {\color[HTML]{FE0000} Standard Windowing {[}512, 512{]}}                       & {\color[HTML]{FE0000} 0.690} & {\color[HTML]{FE0000} 0.698} \\
    {\color[HTML]{6200C9} Linear conversion + resampling + padding {[}704, 704{]}} & {\color[HTML]{6200C9} 0.718} & {\color[HTML]{6200C9} 0.721} \\
    {\color[HTML]{643403} Windowing + resampling + padding {[}704, 704{]}}         & {\color[HTML]{643403} 0.952} & {\color[HTML]{643403} 0.944} \\ \hline
    \end{tabular}
    }
    \caption{}
...
The resampling strategy works best because it standardizes the input size, if we take a look at the values, we can make these two comparisons: 
\begin{itemize}
    \item Linear conversion 512x512 has a mAP of 0.631, while the resampled set arrives to 0.952.
    \item Windowing 512x512 achies a mAP of 0.690 on the test set, while the same set resampled arrives to 0.718.
\end{itemize}
